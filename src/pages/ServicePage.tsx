    ...
    <section className="relative py-28 md:py-36 overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
      ...
      <div className="container mx-auto px-4 relative z-10 text-center">
        <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0}
          className="inline-block px-4 py-1.5 bg-white/10 rounded-full text-xs font-semibold uppercase tracking-widest text-white/80 mb-4 border border-white/20">
          {subtitle}
        </motion.div>
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {features.map((f, i) => (
              <motion.div key={i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={i % 4}
                className="flex items-start gap-3 p-5 bg-card rounded-sm border border-border hover:border-primary/30 hover:shadow-sm transition-all group">
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0}
            ...
            <Link to="/contact"
              className="inline-flex items-center gap-2 px-8 py-3.5 bg-primary text-primary-foreground font-semibold rounded-sm hover:bg-primary/90 transition-all group">
